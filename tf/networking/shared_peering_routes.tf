...
resource "aws_route" "shared_public_to_vpc_peering_rtb0" {
  depends_on = [
  ...
  route_table_id            = data.terraform_remote_state.shared.outputs.public_rt_ids[0]
...
resource "aws_route" "shared_public_to_vpc_peering_rtbl1" {
  depends_on = [
    module.modules_vpc_peering,
    aws_vpc_peering_connection_accepter.peer_prod,
    aws_vpc_peering_connection_accepter.peer_dev,
  ...
  provider                  = aws.shared
  count                     = length(local.vpc_peering_routes) > 0 ? length(local.vpc_peering_routes) : 0
  route_table_id            = data.terraform_remote_state.shared.outputs.public_rt_ids[1]
  destination_cidr_block    = local.vpc_peering_routes[count.index].cidr_block
  vpc_peering_connection_id = local.vpc_peering_routes[count.index].pcx-id
}

resource "aws_route" "shared_public_to_vpc_peering_rtbl2" {
  depends_on = [
    module.modules_vpc_peering,
    aws_vpc_peering_connection_accepter.peer_prod,
    aws_vpc_peering_connection_accepter.peer_dev,
  ...
  provider                  = aws.shared
  count                     = length(local.vpc_peering_routes) > 0 ? length(local.vpc_peering_routes) : 0
  route_table_id            = data.terraform_remote_state.shared.outputs.public_rt_ids[2]
  destination_cidr_block    = local.vpc_peering_routes[count.index].cidr_block
  vpc_peering_connection_id = local.vpc_peering_routes[count.index].pcx-id
}

## Peering Private Routes
resource "aws_route" "shared_private_to_vpc_peering_rtbl0" {
  depends_on = [
    module.modules_vpc_peering,
    aws_vpc_peering_connection_accepter.peer_prod,
    aws_vpc_peering_connection_accepter.peer_dev,
  ...
  provider                  = aws.shared
  count                     = length(local.vpc_peering_routes) > 0 ? length(local.vpc_peering_routes) : 0
  route_table_id            = data.terraform_remote_state.shared.outputs.private_rt_ids[0]
  destination_cidr_block    = local.vpc_peering_routes[count.index].cidr_block
  vpc_peering_connection_id        = local.vpc_peering_routes[count.index].pcx-id
}

resource "aws_route" "shared_private_to_vpc_peering_rtbl1" {
  depends_on = [
    module.modules_vpc_peering,
    aws_vpc_peering_connection_accepter.peer_prod,
    aws_vpc_peering_connection_accepter.peer_dev,
  ...
  provider                  = aws.shared
  count                     = length(local.vpc_peering_routes) > 0 ? length(local.vpc_peering_routes) : 0
  route_table_id            = data.terraform_remote_state.shared.outputs.private_rt_ids[1]
  destination_cidr_block    = local.vpc_peering_routes[count.index].cidr_block
  vpc_peering_connection_id        = local.vpc_peering_routes[count.index].pcx-id
}

resource "aws_route" "shared_private_to_vpc_peering_rtbl2" {
  depends_on = [
    module.modules_vpc_peering,
    aws_vpc_peering_connection_accepter.peer_prod,
    aws_vpc_peering_connection_accepter.peer_dev,
  ...
  provider                  = aws.shared
  count                     = length(local.vpc_peering_routes) > 0 ? length(local.vpc_peering_routes) : 0
  route_table_id            = data.terraform_remote_state.shared.outputs.private_rt_ids[2]
  destination_cidr_block    = local.vpc_peering_routes[count.index].cidr_block
  vpc_peering_connection_id        = local.vpc_peering_routes[count.index].pcx-id
}
#Peering SecureRoutes
resource "aws_route" "shared_secure_to_vpc_peering_rtb0" {
  depends_on = [
    module.modules_vpc_peering,
    aws_vpc_peering_connection_accepter.peer_prod,
    aws_vpc_peering_connection_accepter.peer_dev,
  ...
  provider                  = aws.shared
  count                     = length(local.vpc_peering_routes) > 0 ? length(local.vpc_peering_routes) : 0
  route_table_id            = data.terraform_remote_state.shared.outputs.secure_rt_ids[0]
  destination_cidr_block    = local.vpc_peering_routes[count.index].cidr_block
  vpc_peering_connection_id       = local.vpc_peering_routes[count.index].pcx-id
}

resource "aws_route" "shared_secure_to_vpc_peering_rtbl1" {
  depends_on = [
    module.modules_vpc_peering,
    aws_vpc_peering_connection_accepter.peer_prod,
    aws_vpc_peering_connection_accepter.peer_dev,
  ...
  provider                  = aws.shared
  count                     = length(local.vpc_peering_routes) > 0 ? length(local.vpc_peering_routes) : 0
  route_table_id            = data.terraform_remote_state.shared.outputs.secure_rt_ids[1]
  destination_cidr_block    = local.vpc_peering_routes[count.index].cidr_block
  vpc_peering_connection_id       = local.vpc_peering_routes[count.index].pcx-id
}

resource "aws_route" "shared_secure_to_vpc_peering_rtbl2" {
  depends_on = [
    module.modules_vpc_peering,
    aws_vpc_peering_connection_accepter.peer_prod,
    aws_vpc_peering_connection_accepter.peer_dev,
  ...
  provider                  = aws.shared
  count                     = length(local.vpc_peering_routes) > 0 ? length(local.vpc_peering_routes) : 0
  route_table_id            = data.terraform_remote_state.shared.outputs.secure_rt_ids[2]
  destination_cidr_block    = local.vpc_peering_routes[count.index].cidr_block
  vpc_peering_connection_id        = local.vpc_peering_routes[count.index].pcx-id
}