resource "aws_cloudtrail" "org_cloudtrail" {
  name                          = format("%s-cloudtrail", local.unique_prefix)
  s3_bucket_name                = format("%s-cloudtrail", local.unique_prefix)
  kms_key_id                    = format("arn:aws:kms:%s:%s:alias/security", local.base_region, local.security)
