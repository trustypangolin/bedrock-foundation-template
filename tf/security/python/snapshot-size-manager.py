...
def core_functions(regions, accounts, role):
  for account in accounts:
    ...
    for region in regions:
      ...
      snapshot_size_manager(sessions, region)
...
def snapshot_size_manager(s, region):
  ...
  jq = []
  for ind in range(len(snaps)):
    if snaps[ind][0] not in str(jq) and snaps[ind][0] != 'vol-ffffffff':
      jq.append((snaps[ind][0], snaps[ind][1]))

  for i in range(len(jq)):
    v = jq[i][0]
    sid = jq[i][1]
    print('first snapshot:', sid, '-- message for sqs:', i, json.dumps({'volume': v}))
    response = sqs.send_message(
      QueueUrl=queue_url,
      DelaySeconds=0,
      MessageBody=json.dumps({'account': account, 'region': region, 'volume': v}),
      )
